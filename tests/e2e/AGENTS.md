...
Keep browser-test work inside `tests/e2e/`. Do not add application-only test routes, authentication bypasses, special headers, or runtime flags. Exercise genuine local Supabase sessions and replace only external delivery systems, such as SendGrid, with local fakes.
